 ...
 */
public class MyLog {
    private static Boolean MYLOG_SWITCH = true;// 日志文件总开
    private static Boolean MYLOG_WRITE_TO_FILE = true;// 日志写入文件开关  
    private static char MYLOG_TYPE = 'v';// 输入日志类型，w代表只输出告警信息等，v代表输出所有信息  
    private static String MYLOG_PATH_SDCARD_DIR = "/sdcard/";// 日志文件在sdcard中的路径  
    private static int SDCARD_LOG_FILE_SAVE_DAYS = 0;// sd卡中日志文件的最多保存天数  
    private static String MYLOGFILEName = "职场护航Log.txt";// 本类输出的日志文件名称  
...
//                String needWriteMessage = myLogSdf.format(nowtime)+ "    " + mylogtype
//                        + "    " + tag + "    " + text;
                File file = new File(MYLOG_PATH_SDCARD_DIR, needWriteFiel +"职场护航所有系统日志.txt");
                try {
                    FileWriter filerWriter = new FileWriter(file, true);//后面这个参数代表是不是要接上文件中原来的数据，不进行覆盖  
                    BufferedWriter bufWriter = new BufferedWriter(filerWriter);
                    bufWriter.write(str);
    ...
    public static String getSDPath(){
        ...
        return sdDir.toString()+"/laodongshebao/log";
    }
